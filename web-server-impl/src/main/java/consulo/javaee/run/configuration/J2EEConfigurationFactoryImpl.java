...
public class J2EEConfigurationFactoryImpl extends J2EEConfigurationFactory {
    @Override
    public RunConfiguration createJ2EERunConfiguration(
    ...
    @Override
    public RunnerAndConfigurationSettings addAppServerConfiguration(
        @Nonnull Project project,
        @Nonnull ConfigurationFactory type,
        @Nonnull ApplicationServer appServer
    ) {
        return null;
    }
    ...
    public ConfigurationFactory createFactory(J2EEConfigurationType type, boolean isLocal, String id, LocalizeValue name) {
        return new JavaEEConfigurationFactoryImpl(
            type,
            id,
            name,
            isLocal ? type.getLocalIcon() : type.getRemoteIcon(),
            isLocal,
            type.getBundleType()
        );
    }
